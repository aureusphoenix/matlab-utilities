function map = overgray(dis,limit)
...
%% Main code
    ...
    % lower limit blue
    map(1:ceil(limit(1)*dis/100),1) = 0;
    map(1:ceil(limit(1)*dis/100),2) = 0;
    map(1:ceil(limit(1)*dis/100),3) = 1;
    
    % upper limit red
    map(dis-floor(limit(2)*dis/100):dis,1) = 1;
